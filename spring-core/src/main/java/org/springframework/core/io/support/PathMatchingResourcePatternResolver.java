...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 * Ant 路径匹配器 支持 Ant 风格的路径匹配模式（类似于 "**\\/*.xml"）。
	 */
	...
	/**
	 * 该方法，直接委托给相应的 ResourceLoader 来实现。所以，如果我们在实例化的 PathMatchingResourcePatternResolver 的时候，
	 * 如果未指定 ResourceLoader 参数的情况下，那么在加载资源时，其实就是 DefaultResourceLoader 的过程。
	 * @return
	 */
	@Override
	@Nullable
	...
	/** 找到根目录
	 *	例子
	 * @example  原:classpath*:test/cc*\\/spring-*.xml	确定的根目录: classpath*:test/
	 * @example  原:classpath*:test/aa/spring-*.xml		确定的根目录: classpath*:test/aa
	 *
